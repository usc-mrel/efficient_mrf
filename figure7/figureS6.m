...
fS6 = figure('Position', [200 160 925 605], 'Color', 'k', 'InvertHardcopy', 'off');
...
%% T1 maps.
fS6_t1 = figure('Position', [200 160 925 605], 'Color', 'k', 'InvertHardcopy', 'off');
[ha, pos] = tight_subplot(2, 3, [0 0], [0 0], [0.03 0.03]);
ha = (reshape(ha, [3 2])).';
...
for ii = [14 11 3]
    ...
    ncol = ncol + 1;
    axes(ha(1, ncol));
    imagesc(reorient(mask .* t1map), [0 1.5e3]);
    axis off image;
    hold on;
    plot(window_x, window_y, '--', 'LineWidth', 2, 'Color', [0 255 128] / 255);
    text(60, 70, sprintf('%g ms', ro(15 - ii)), 'Color', text_color, 'FontSize', 13, 'FontWeight', 'bold');
    ...
    axes('Position', [(0.18 + (ncol - 1) * 0.3133) 0.5  0.2 0.2]);
    imagesc(reorient(mask(xrange, yrange) .* t1map(xrange, yrange)), [0 1.5e3]);
    axis off image;
    hold on;
    plot(inset_x, inset_y, '-', 'LineWidth', 2, 'Color', [0 255 128] / 255);
...
%% T2 maps.
fS6_t2 = figure('Position', [200 160 925 605], 'Color', 'k', 'InvertHardcopy', 'off');
[ha, pos] = tight_subplot(2, 3, [0 0], [0 0], [0.03 0.03]);
ha = (reshape(ha, [3 2])).';
...
for ii = [14 11 3]
    ...
    ncol = ncol + 1;
    axes(ha(1, ncol));
    imagesc(reorient(mask .* t2map), [0 2e2]);
    axis off image;
    hold on;
    plot(window_x, window_y, '--', 'LineWidth', 2, 'Color', [0 255 128] / 255);
    text(60, 70, sprintf('%g ms', ro(15 - ii)), 'Color', text_color, 'FontSize', 13, 'FontWeight', 'bold');
    ...
    axes('Position', [(0.18 + (ncol - 1) * 0.3133) 0.5  0.2 0.2]);
    imagesc(reorient(mask(xrange, yrange) .* t2map(xrange, yrange)), [0 2e2]);
    axis off image;
    hold on;
    plot(inset_x, inset_y, '-', 'LineWidth', 2, 'Color', [0 255 128] / 255);
...
movie_not_used = figure('Position', [200 160 1235 625], 'Color', 'k', 'InvertHardcopy', 'off');
[ha, pos] = tight_subplot(1, 2, [0 0], [0 0], [0.03 0.03]);
ha = (reshape(ha, [2 1])).';

for ii = [14 11 8 5 3 1]
    ...
    im = frame2im(getframe(fS7));
    [A, map] = rgb2ind(im,256);
    if ii == 14
        imwrite(A, map, 'movie_not_used.gif', 'gif', 'LoopCount', Inf, 'DelayTime', 0.5);
    else
        imwrite(A, map, 'movie_not_used.gif', 'gif', 'WriteMode', 'append', 'DelayTime', 0.5);
    end
end

%% Save figures.
saveas(fS6, 'figureS6.fig');
saveas(fS6, 'figureS6.png');
saveas(fS6_t1, 'figureS6_t1.fig');
saveas(fS6_t1, 'figureS6_t1.png');
saveas(fS6_t2, 'figureS6_t2.fig');
saveas(fS6_t2, 'figureS6_t2.png');
