%% MRF pattern matching script for pseudo-replicas.
clear; clc; close all;
...
opt_data = '';
tstart = tic;
for opt_indx = [1 3 5 8 11 14]
    slice_loc = 36; % Works for one 2D slice a time. Not for 3D volume.
    dict_folder = sprintf('../../data/dictionaries/disc/pulseq_readout_experiments/dict_ni%d_nt%d%s', Ni(opt_indx), Nt(opt_indx), opt_dict);
    
    fprintf('Loading dictionaries with B1+ correction for Ni = %d...\n', Ni(opt_indx));
    ii = 1;
    for rf = 0.5:0.05:1.5
    ...
    for nr_replications = 0:99
        mat_names{1} = fullfile(data_dir, 'nufft_3d/replicas', sprintf('pulseq_mrf_slice%d__fa75_cycle2_ni%d_nt%d%s_rep%d.mat', slice_loc, Ni(opt_indx), Nt(opt_indx), opt_data, nr_replications));
        mat_names{2} = fullfile(data_dir, 'subspace_3d/replicas', sprintf('pulseq_mrf_subspace_slice%d_B1__fa75_cycle2_ni%d_nt%d%s_rep%d.mat', slice_loc, Ni(opt_indx), Nt(opt_indx), opt_data, nr_replications));
        mat_names{3} = fullfile(data_dir, 'maxgirf_3d/replicas', sprintf('pulseq_mrf_maxgirf_slice%d_B0__fa75_cycle2_ni%d_nt%d%s_rep%d.mat', slice_loc, Ni(opt_indx), Nt(opt_indx), opt_data, nr_replications));
