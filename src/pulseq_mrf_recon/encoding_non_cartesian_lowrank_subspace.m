function outp = encoding_non_cartesian_lowrank_subspace(inp, csm, Omega, Phi, nufft_st, w, transpose_indicator)
...
for c = 1:Nc
    ...
    %----------------------------------------------------------------------
%     FScU = complex(zeros(Np, Lt, 'double')); % Np x Lt
...
%     end
    ...
    FScU = nufft(repmat(csm(:, :, c), [1 1 Lt]) .* U, nufft_st{1, 1}) * scale_factor;
...
%     end
    ...
    % Verified optimization, ~7% computation time reduction, Zhibo.
    AcHb = nufft_adj(FscUPhiPhiH .* repmat(w(:), [1 Lt]), nufft_st{1, 1}) * scale_factor;
    %----------------------------------------------------------------------
