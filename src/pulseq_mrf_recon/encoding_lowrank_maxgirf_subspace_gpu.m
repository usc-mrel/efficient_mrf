function outp = encoding_lowrank_maxgirf_subspace_gpu(inp, csm, Omega, Phi, Ue, Ve, w, st, transpose_indicator)
% Modified by Zhibo.
...
%         end
        ...
        EScU = complex(zeros(Ns, Lt, 'double', 'gpuArray'));
        for ell = 1 : Ls
            ScU = repmat(csm(:, :, c), [1 1 Lt]) .* U;
%             ScU = reshape(ScU, [N Lt]);
            EScU = EScU + repmat(Ue(:, ell, i), [1 Lt]) .* nufft_gpu(repmat(reshape(conj(Ve(:,ell,i)), [N1 N2]), [1 1 Lt]) .* ScU, st{i, 1}) * scale_factor;
        end
        ...
        %----------------------------------------------------------------------
        EScUPhi = complex(zeros(Np, M, 'double', 'gpuArray')); % Np x M
...
%         end
        ...
        AHd = complex(zeros(N1 * N2, Lt, 'double', 'gpuArray'));
        for ell = 1 : Ls
            FHDuHd = nufft_adj_gpu((repmat(conj(Ue(:, ell, i)), [1 Lt]) .* EscUPhiPhiH) .* repmat(w(:, i), [1 Lt]), st{i, 1}) * scale_factor;
