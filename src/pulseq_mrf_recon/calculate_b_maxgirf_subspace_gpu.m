function b = calculate_b_maxgirf_subspace_gpu(Phi, Ue, Ve, d, Omega, csm, st, w)%% Calculate b: A(u) = b
%--------------------------------------------------------------------------
...
d = gpuArray(permute(d, [1 4 2 3])); % Nk x M x Narms x Nc
...
%         end
        ...
        for ell = 1 : Ls
            FHDuHd = nufft_adj_gpu((repmat(conj(Ue(:, ell, i)), [1 Lt]) .* FscUPhiPhiH) .* repmat(w(:, i), [1 Lt]), st{i, 1}) * scale_factor;
