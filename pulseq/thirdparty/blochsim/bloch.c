...
/*-----------------------------------------------------------------------*/
/* Function takes the given endtimes of intervals, and returns the       */
/* interval lengths in an array, assuming that the first interval starts */
/* at 0.                                                                 */
/*                                                                       */
/* If the intervals are all greater than 0, then this returns 1,         */
              ...
              double *mx, double *my, double *mz, int mode)
{
    ...
    {
        /* Rotation */
        rotz = -(*xgrad++ * gammadx + *ygrad++ * gammady + *zgrad++ * gammadz + df * TWOPI) * *tsteps;
        rotx = (- *b1real++ * GAMMA * *tsteps);
        //roty = (+ *b1imag++ * GAMMA * *tsteps++);
        roty = (- *b1imag++ * GAMMA * *tsteps++); /* NGL: changed to minus */
        ...
        for (poscount = 0; poscount < npos; poscount++)
        {
            ...
            {	/* First go through and find steady state, then
                ...
                blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime,
                        e1, e2, *dfreq, *dxptr, *dyptr, *dzptr, mx, my, mz, 1);

                blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime,
                        e1, e2, *dfreq, *dxptr++, *dyptr++, *dzptr++, mx, my, mz, 2);
            } else
            {
                blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime,
                        e1, e2, *dfreq, *dxptr++, *dyptr++, *dzptr++, mx, my, mz, mode);
            }
