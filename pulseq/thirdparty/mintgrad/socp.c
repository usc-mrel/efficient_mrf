...
)
{
    ...
    *pdev = 2 * L * (log(*pgap) - log((double) L));
    for (i = 0, k = 0; i < L; ++i)
    {
...
)
{
    ...
    ptrdiff_t la_info;  /* exit info from dposvx_ and dgelss_ */
    ...
    /* for dgelss_() */
    ptrdiff_t rank;
    ptrdiff_t lwork = 10 * n;   /* size of workspace, minimum is 5*n */
    ...
    */
    ...
    /* outer loop */
    iter_out = 0;
    while (!(( rel_tol < 0.0 && (xtarget = (ddot_(&n, f, &int1, x, &int1) < target || -ddot_(&m, b, &int1, z, &int1) >= target)) )
          || ( abs_tol > 0.0 && (xabs = (gap <= abs_tol)) )
          || ( rel_tol > 0.0 && (xrel = (((lbound = -ddot_(&m, b, &int1, z, &int1)) > 0.0 && gap / lbound <= rel_tol) || ((lbound = -ddot_(&n, f, &int1, x, &int1)) > 0.0 && gap / lbound <= rel_tol))) )
          || ( xiter = (iter_out >= *iter) )))
    {
        ++iter_out;
